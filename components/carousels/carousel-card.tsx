...
interface CarouselCardProps {
  data: any; // Replace 'any' with the actual type if known, e.g. 'YourDataType'
}
      ...
      {data?.fields?.Screenshots && (
        <ContextMenuCard>
          <div className="group relative flex flex-col gap-y-3 md:gap-y-4">
            ...
            {data?.fields?.Screenshots && (
              <Image
                src={data?.fields?.Screenshots[0].url}
                alt="phone screen"
                width={300}
                height={800}
              ...
              >
                <CarouselContent className="m-0">
                  {data?.fields?.["S3 Links"]
                    ...
                    : data?.fields?.Screenshots?.map(
                        (s: { url: string | StaticImport }, index: number) => (
                          <CarouselItem key={index} className="px-7">
                            <Image
                              src={s.url}
                              alt={`phone screen ${index + 1}`}
